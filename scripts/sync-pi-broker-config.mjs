...
const [authFile, modelsFile, modelsPatchFile, ...authPatchFiles] = process.argv.slice(2);

if (!authFile || !modelsFile || !modelsPatchFile) {
  console.error("Usage: sync-pi-broker-config.mjs <auth.json> <models.json> <models-patch.json> [auth-patch...]");
  process.exit(1);
}
