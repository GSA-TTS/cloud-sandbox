...
* **Dynamic Credential Binding (`vcap-env.js`):** Because Backstage expects traditional environment variables (like `POSTGRES_USER` and `CLOUDGOV_CLIENT_ID`), but cloud.gov uses the `VCAP_SERVICES` JSON block, we use a pre-load script (`node -r ./vcap-env.js ...`) in the manifest start command. This script dynamically parses `VCAP_SERVICES` to locate a bound AWS RDS service and a bound cloud.gov identity provider service, then injects standard env vars into the Node process before Backstage initializes.
...
* The application manifest (`manifest.yml`) and `app-config.production.yaml` now use placeholders and environment variables for the public route.
...
* The `cloud-gov-identity-provider` should be brokered with a deployment-specific service instance name and callback route outside this repo.

## 4. Next Steps for Incoming Agent

1. **Validate OIDC Authentication Flow:** Once the portal starts, attempt to log in via cloud.gov SSO. Check the Cloud Foundry logs for the deployed app to ensure the callback routing completes successfully and identity tokens map properly to Backstage users.
